<template>
  <div class="simple-player">
    <div class="player-header">
      <h3>{{ fileName }}</h3>
      <el-tag v-if="isLoadingFfmpeg" type="warning" effect="light" size="small" style="margin-left: 10px;">加载解码器...</el-tag>
      <el-tag v-if="isProcessing" type="warning" effect="light" size="small" style="margin-left: 10px;">处理中 {{ processingProgress.toFixed(1) }}%...</el-tag>
      <el-tag v-if="isPlayingWithMSE" type="success" effect="light" size="small" style="margin-left: 10px;">MSE 播放中</el-tag>
    ...
    <div class="video-container">
      <video
        ...
        controls
        autoplay
        muted crossorigin="anonymous" class="video-element"
        @error="handleVideoError" >
        <track
          v-if="subtitleUrl"
          label="默认字幕"
          kind="subtitles"
          srclang="und" :src="subtitleUrl"
          default
        />
      </video>
    ...
    <div v-if="showFallbackOptions" class="fallback-prompt">
      <el-tooltip content="使用内置MSE播放器 (可能需要处理时间，需显卡支持视频解码)" placement="top">
        <el-button type="primary" :loading="isLoadingFfmpeg || isProcessing" @click="tryMSEPlayback">
          尝试内置播放 (MSE)
...
async function loadFfmpeg(): Promise<boolean> {
  if (ffmpegRef.value?.loaded) return true // 检查 .loaded 状态
  ...
  try {
    ...
    ffmpeg.on('progress', ({ progress, time }) => {
      if (isProcessing.value) {
        processingProgress.value = progress * 100;
        // console.log(`[FFmpeg progress] ${processingProgress.value.toFixed(1)}%`);
      }
    });
    ...
    await ffmpeg.load({
      ...
      // workerURL: await toBlobURL(`${FFMPEG_BASE_URL}/ffmpeg-core.worker.js`, 'text/javascript'), // 可选的 Worker
    });
    ...
    errorMessage.value = `加载 FFmpeg 核心失败: ${error.message}`
    return false
  } finally {
...
async function handleSourceOpen() {
  ...
  try {
    ...
    const ffmpegRemuxArgs = [
      '-i', inputFileName,
      '-map', '0:v:0?', // 映射第一个视频流 (如果存在)
      '-map', '0:a:0?', // 映射第一个音频流 (如果存在)
      '-c:v', 'copy',   // 尝试复制视频编解码器
      '-c:a', 'copy',   // 尝试复制音频编解码器
      '-movflags', 'frag_keyframe+empty_moov+default_base_moof', // MSE 关键参数
      '-f', 'mp4',      // 输出 MP4 格式
      outputFileName
    ];
      ...
      if (!sourceBuffer.value?.updating && mediaSource.value?.readyState === 'open') {
          console.log('数据追加完毕，调用 endOfStream。');
           // 确保在调用 endOfStream 之前没有其他挂起的更新
            try {
                if(mediaSource.value.readyState === 'open') { // Double check state
    ...
    try {
      ...
      // 字幕提取失败不应阻止视频播放
      ElNotification.warning({ title: '字幕提取失败', message: '未能提取内嵌字幕，将无字幕播放。', duration: 3000 });
    ...
    if(!(error.message.includes("subtitle") || error.message.includes("字幕"))){ // 如果错误不是字幕相关的
...
const openSystemPlayer = async (): Promise<void> => {
  ...
  // 检查播放器路径设置
  if (!ConfigStore.PathConfig.playerPath || !window.nodeAPI.fs.existsSync(ConfigStore.PathConfig.playerPath)) {
